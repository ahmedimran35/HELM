...
export function MemoryStrategiesPage() {
  ...
  return <div className="p-6 max-w-[1100px] space-y-6">
    ...
    <div className="border border-border bg-panel"><div className="px-4 py-2 border-b border-borderSoft mono-caps text-[11px] text-textMuted">Configured ({list.length})</div>
      {list.length === 0 ? <div className="p-6 text-center mono-caps text-[11px] text-textFaint">no strategies</div> : list.map(s => <div key={s.id} className="px-4 py-3 border-b border-borderSoft last:border-0 flex items-center gap-3"><div className="flex-1"><div className="font-mono text-[13px] text-text">{s.kind}</div><div className="mono-caps text-[10px] text-textMuted">{s.scope} · {s.scope_id ?? "all"} · priority {s.priority}</div></div><button type="button" aria-label={`Enable ${s.kind}`} onClick={() => apiPatch(`/memory/strategies/${s.id}`, { enabled: !s.enabled }).then(reload)} className={`mono-caps text-[10px] px-2 py-1 border ${s.enabled ? "border-teal/50 text-teal" : "border-border text-textFaint"}`}>{s.enabled ? "enabled" : "disabled"}</button>{s.kind === "summary" && <Button size="sm" onClick={() => summarize(s.id)}>Summarize now</Button>}<Button variant="danger" size="sm" onClick={() => apiDelete(`/memory/strategies/${s.id}`).then(reload)}>Delete</Button></div>)}
    </div>
  </div>;
}
